...
def send_rtg_item_report(
    ...
    """
    Send RTG item data report via email
    
    Reuses the SAME functions as:
    - /api/item-summary-html/ (uses json_to_html_table)
    - /api/item-summary-excel/ (uses json_to_excel)
    ...
    try:
        ...
        #     shift=shift
        # )
        report_result = get_item_summary_report(str(report_date))
        ...
        html_table = item_summary_to_html(report_data)
        ...
        logger.info("📑 Generating Excel")
        # excel_bytes = json_to_excel(report_data)
        excel_bytes = item_summary_to_excel(report_data)
        ...
        # ✅ Send email
        msg = EmailMessage()
        msg['Subject'] = f'RTG Data item: {report_date.strftime("%d-%b-%Y")} ({mail_topic_shift})'
        msg['From'] = send_email
        msg['To'] = to_email if isinstance(to_email, str) else ', '.join(to_email)
        ...
        msg.add_attachment(
            ...
            maintype='application',
            subtype='vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            filename=f'item-report-{report_date.strftime("%Y-%m-%d")}.xlsx'
        )
